...
func DeployCatalog(ctx context.Context, opts catalogutils.OpenshiftConfigureOptions) error {
	...
	// Step 1: Fetch the operation timeout from metadata
	timeout, err := getOperationTimeout(ctx, tp)
	if err != nil {
		return err
	}
...
func getOperationTimeout(ctx context.Context, tp templates.Template) (time.Duration, error) {
